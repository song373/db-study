...
INSERT INTO seq_test VALUES((select COUNT(*)+1 from seq_test), '이름2');
select * from seq_test;
...
select NVL (MAX(no)+1, 1) from seq_test;
...
DROP SEQUENCE seq_seq_tesk_pk;

CREATE SEQUENCE seq_seq_tesk_pk
INCREMENT BY 1
START WITH 1;

select seq_seq_tesk_pk.nextval from dual;

--다음 시퀀스 값 조정

--현재값 확인
select seq_seq_tesk_pk.currval from dual;
--다음에 불렀을때 1이 될수 있게 값 조정
ALTER SEQUENCE seq_seq_tesk_pk INCREMENT BY -9;
ALTER SEQUENCE seq_seq_tesk_pk MINVALUE 0;
select seq_seq_tesk_pk.nextval from dual;
--다시 원상 복구
ALTER SEQUENCE seq_seq_tesk_pk INCREMENT BY 1;
--시퀀스 다시 사용
select seq_seq_tesk_pk.nextval from dual;
